...
def summarize_budget_means_from_csv_folder(folder_path: str, output_dir: str):
    """
    폴더 내 CSV 파일들을 예산/결산, 세입/세출로 분류하여 각각 합계 요약을 저장합니다.
    저장 파일명 앞에는 public/private 구분이 포함됩니다.

    Args:
        folder_path (str): CSV 파일들이 있는 폴더 경로
        output_dir (str): 합계 요약 결과를 저장할 디렉토리
    """
    ...
    for key, records in result_dict.items():
        if records:
            output_filename = f"{prefix}_{key}_합계요약.csv"  # 여기서 prefix 추가됨!
            ...
            sum_rows = []
            for level, recs in school_level_frames.items():
                if recs:
                    df_level = pd.DataFrame(recs)
                    sum_vals = df_level.drop(columns=["파일명", "학교급"]).sum()
                    sum_row = {"파일명": f"{prefix}_{key}_{level}_합계", "학교급": level}
                    sum_row.update(sum_vals.to_dict())
                    sum_rows.append(sum_row)

            # 연도별 합계 추가
            year_frames = {}
            for record in result_dict[key]:
                year_match = re.search(r"(\d{4})", record["파일명"])
                if year_match:
                    year = year_match.group(1)
                    if year not in year_frames:
                        year_frames[year] = []
                    year_frames[year].append(record)

            for year, recs in year_frames.items():
                if recs:
                    df_year = pd.DataFrame(recs)
                    sum_vals = df_year.drop(columns=["파일명", "학교급"]).sum()
                    sum_row = {"파일명": f"{year}_합계", "학교급": None}
                    sum_row.update(sum_vals.to_dict())
                    sum_rows.append(sum_row)

            # 최종 결과 저장
            df_all = pd.DataFrame(result_dict[key])
            df_final = pd.concat([df_all, pd.DataFrame(sum_rows)], ignore_index=True)
...
def summarize_total_and_school_level_mean(input_dir: str, output_dir: str, prefix: str):
    ...
    전체 합계와 학교급별 합계를 구해 summary 폴더에 저장
    ...
    # ✅ 전체 합계 계산
    total_sum = df_all[value_columns].sum()
    total_sum_row = {"파일명": f"{prefix}_전체합계"}
    total_sum_row.update(total_sum)
    df_total = pd.DataFrame([total_sum_row])
    ...
    for level in ["초등", "중등", "고등"]:
        ...
        if not sub_df.empty:
            sum_row = {"파일명": f"{prefix}_{level}_합계"}
            sum_row.update(sub_df[value_columns].sum())
            school_rows.append(sum_row)
    df_school = pd.DataFrame(school_rows)

    # ✅ 저장
    os.makedirs(output_dir, exist_ok=True)
    df_total.to_csv(os.path.join(output_dir, f"{prefix}_전체합계요약.csv"), index=False, encoding="utf-8-sig")
    df_school.to_csv(os.path.join(output_dir, f"{prefix}_학교급별합계요약.csv"), index=False, encoding="utf-8-sig")
    print(f"✅ {prefix} 전체/학교급 합계 요약 저장 완료")
...
def combine_public_private_summary_and_average(
    ...
    """
    private & public 요약 파일을 병합하여 각 예산/결산 - 세입/세출 별 합계를 구해 저장합니다.
    ...
    for combo in combinations:
        private_path = os.path.join(private_dir, f"private_{combo}_합계요약.csv")
        public_path = os.path.join(public_dir, f"public_{combo}_합계요약.csv")
        ...
        row = {"파일명": f"combined_{combo}_합계"}
        for col in value_cols:
            row[col] = df_combined[col].sum()

        # 저장
        df_output = pd.DataFrame([row])
        output_path = os.path.join(output_dir, f"combined_{combo}_합계요약.csv")
