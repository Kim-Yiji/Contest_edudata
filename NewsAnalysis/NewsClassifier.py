...
def classify_news(input_file, output_file):
    """뉴스 분류 함수"""
    # 출력 디렉토리 설정
    classified_dir = os.path.dirname(output_file)
    ...
    # 예산 항목 데이터 로드
    budget_keywords_df = pd.read_csv('NewsAnalysis/BaseData/세부항목예시별 키워드 확장 (LLM 기반).csv', encoding='utf-8')

    # 중분류와 대분류 정보 매핑 생성 (세부항목예시별 키워드 확장 (LLM 기반).csv에서 직접 사용)
    ...
    def prepare_news_text(row):
        ...
        return f"{title} {feature_sentence}"
...
if __name__ == "__main__":
    ...
    classify_news(input_file, output_file) 